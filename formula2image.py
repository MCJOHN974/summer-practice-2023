...
SETUP = [BASIC_SKELETON, "convert -density 200 -quality 100 %s.pdf %s.png", lambda filename: os.path.isfile(filename + ".png")]
...
def remove_temp_files(name):
    ...
    os.remove(name+".log")
    os.remove(name+".pdf")
    os.remove(name+"_crop.pdf")
    os.remove(name+".tex")


def remove_all_temp_files(dir):
    call([f'rm {dir}/*.aux {dir}/*.log {dir}/*.pdf {dir}/*_crop.pdf {dir}/*.tex'], shell=True, stdout=DEVNULL, stderr=DEVNULL)


def formula_to_image_simplified(formula):
    formula = formula.strip("%")
    name = hashlib.sha1(formula.encode('utf-8')).hexdigest()[:15]

    rend_setup = SETUP
    rend_name = 'basic'
    working_dir = f"images_{int(name, 16) % 10}"
    full_path = f"images_{int(name, 16) % 10}/{name}_{rend_name}"
    ...
    code = call(["pdflatex", '-interaction=nonstopmode', '-halt-on-error', '-output-directory', working_dir, full_path + ".tex"],
                stdout=DEVNULL, stderr=DEVNULL)
    if code != 0:
        os.system("rm -rf " + full_path + "*")
        return None
    call([f"pdfcrop", '-margin', '3', full_path + '.pdf', full_path + '_crop.pdf'], stdout=DEVNULL, stderr=DEVNULL)
    call(['cp', full_path + '_crop.pdf', full_path + '.pdf'], stdout=DEVNULL, stderr=DEVNULL)
    ...
    if random.random() > 0.95:
        try:
            remove_all_temp_files(working_dir)
...
if __name__ == '__main__':
    ...
    formula_to_image_simplified('\\int _ { a } ^ { e } x ^ 2 d x')
